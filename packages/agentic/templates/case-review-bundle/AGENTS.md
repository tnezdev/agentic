...
- `agentic approve . --action <action-id> --principal user:reviewer.alba` records a durable grant, resumes the stored `external.handoff` payload through the local handler, and writes a released `handoff-note` artifact.
